$(document).ready(function () {
	...
	var common = nem.model.objects.create("common")("Test7978", "c10c40835186ae027c9b23f8be06bfc5493ed3394fae5f5e5a8689bafbbbcaf8");
	var mosaicDefinitions = nem.model.objects.get("mosaicDefinitionMetaDataPair");

	var savedScores = JSON.parse(window.localStorage.getItem("random-gamee"));
	var scores;
  ...
  function save() {
    ...
    transferTransaction.message = "Random game";
		...
		var mosaicAttachment = nem.model.objects.create("mosaicAttachment")("achin", "score", result);
		...
		nem.com.requests.namespace.mosaicDefinitions(endpoint, mosaicAttachment.mosaicId.namespaceId).then(function(res){
		  ...
		  nem.model.transactions.send(common, preparedTransaction, endpoint).then(function(res){
				if (res.code >= 2) {
					...
					window.localStorage.setItem("random-gamee", JSON.stringify(scores));
					updateScoringTable();
				}
			  }, function(err){
			    console.log(err);
